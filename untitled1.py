...
with open('RossTaylor.csv' ,'r') as t:
    ...
    for i in csv_reader:
        ...
        if runs > year_wise_runs[str(year)]:
             year_wise_runs[str(year)] = runs
             
    print(year_wise_runs)
    
    a1 = list(year_wise_runs.keys())
    a2 = list(year_wise_runs.values())
    a3 = np.arange(len(a1))
    plt.bar(a3,a2,tick_label = a1)
    plt.show()
    l1 = []  
    year_wise_runs2 = {}
    for i in csv_reader:
